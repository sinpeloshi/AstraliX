...
<head>
    ...
    <style>
        ...
        .header-ax h5 { font-weight: 800; letter-spacing: 1.5px; margin: 0; color: #0F172A; font-size: 1.4rem; text-transform: none; }
        ...
        .bottom-bar { background: rgba(255,255,255,0.85); backdrop-filter: blur(12px); position: fixed; bottom: 0; width: 100%; height: 85px; display: flex; justify-content: space-around; align-items: center; border-top: 1px solid rgba(0,0,0,0.05); }
        .nav-link-ax { color: #94A3B8; text-align: center; text-decoration: none; flex: 1; font-size: 10px; font-weight: 600; display: flex; flex-direction: column; align-items: center; gap: 6px; }
...
<body>
    ...
    <div class="bottom-bar">
        <a class="nav-link-ax active" onclick="location.reload()"><i class="fas fa-chart-pie"></i>Overview</a>
        <a class="nav-link-ax" style="opacity:0.5"><i class="fas fa-paper-plane"></i>Transfer</a>
        <a class="nav-link-ax" style="opacity:0.5"><i class="fas fa-shield-halved"></i>Vault</a>
    </div>
    <script>
        ...
        async function mine() {
            ...
            if(r.ok) { alert("¡Mined!"); load(); } else { alert("Network error."); }
        }
