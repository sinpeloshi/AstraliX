...
// ==========================================
// 🎨 FRONTEND INTEGRADO (PREMIUM & RESPONSIVE)
// ==========================================
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .c-title { font-weight: 700; margin-bottom: 10px; font-size: 0.75rem; letter-spacing: 1px; text-transform: uppercase; }
...
<head>
    ...
    <style>
        ...
        .block-idx { background: #E0E7FF; padding: 5px 12px; border-radius: 100px; font-weight: 800; font-size: 0.7rem; color: var(--primary); }
...
<body>
    <div class="header-ax"><h5>AstraliX Core</h5><div class="status-box"><span class="status-dot"></span><span class="status-text">NODE SYNCHRONIZED</span></div></div>
    <div id="v-dash" class="view-ax" style="display:flex;">
        ...
        <div class="card-ax"><span class="balance-label">Treasury Reward Pool</span><div id="pool-txt" class="balance-amount" style="color:var(--primary); font-size:1.8rem;">0.00 AX</div><div class="pill-address">AXf7ca3d5889ed99de642913af6c5630d6c491732b44180771cba042a4eb5a7109cc3ccde9e1a24d5315947415d5e592123ab90edcc4ea85415c1747fbe1684158</div></div>
        <button class="btn-ax" onclick="mine()">VALIDATE NETWORK (+50 AX)</button>
    </div>
    <div id="v-wallet" class="view-ax">
        <div class="card-ax">
            <span class="balance-label">Internal Asset Transfer</span>
            <input type="text" id="tx-to" style="width:100%; padding:18px; border-radius:18px; border:2px solid #E2E8F0; background:#F8FAFC; margin-bottom:12px; outline:none;" placeholder="Recipient Address (AX...)">
            <input type="number" id="tx-amt" style="width:100%; padding:18px; border-radius:18px; border:2px solid #E2E8F0; background:#F8FAFC; margin-bottom:20px; outline:none;" placeholder="Amount AX">
    ...
    <div id="v-sec" class="view-ax">
        <div class="card-ax">
            <span class="balance-label">Vault Access Point</span>
            <textarea id="i-seed" style="width:100%; background:#F8FAFC; border:2px solid #E2E8F0; border-radius:20px; padding:20px; font-size:0.95rem; resize:none; outline:none;" rows="3" placeholder="Input your 24-word recovery phrase..."></textarea>
            ...
            <div id="g-res" style="display:none; margin-top:30px; text-align:left;">
                <span style="font-size:0.75rem; font-weight:800; color:#EF4444; text-transform:uppercase;">Recovery Seed</span>
    ...
    <script>
        ...
        async function renderExplorer() {
            ...
            for(let i=0; i<revChain.length; i++) {
                ...
                let ts = b.Timestamp || b.timestamp; let hash = b.Hash || b.hash;
                let txs = b.Transactions || b.transactions || [];
                html += '<div class="block-card"><div class="block-header"><span class="block-idx">BLOCK #' + idx + '</span></div><div class="block-hash">' + hash + '</div><div style="font-size:0.6rem; color:#94A3B8; margin-top:10px;">TX COUNT: ' + txs.length + ' • SHA-512 SECURED</div></div>';
            }
            list.innerHTML = html;
        }
        ...
        async function gen() {
            ...
            let seedHtml = ""; for(let i=0; i<seed.length; i++) seedHtml += '<div style="font-size:0.75rem; background:white; padding:10px; border-radius:12px; color:#475569; font-weight:600; border:1px solid #F1F5F9;"><span style="color:#CBD5E1; font-size:0.6rem; margin-right:8px;">'+(i+1)+'</span>'+seed[i]+'</div>';
