...
# --- 📡 ESCUADRÓN DE 40 OBJETIVOS (Tendencias 2026) ---
TOKENS = {
    # Memes & Dexscreener Trending
...
def check_commands():
    ...
    try:
        ...
        for update in r.get("result", []):
            ...
            if txt == "/status":
                notify(f"🛰️ *AstraliX:* Escaneando {len(TOKENS)} objetivos...\n⚡ Modo: Agresivo", buttons=True)
            elif txt == "/balance":
                ...
                notify(msg if ultimos_datos else "Calculando Matrix...", buttons=True)
            elif txt == "/retiro":
                ejecutar_retiro()
    except: pass
...
def execute_strike(r1, r2, t_addr, t_name, profit):
    ...
    try:
        ...
        s = w3.eth.account.sign_transaction(tx, PRIV_KEY)
        h = w3.eth.send_raw_transaction(s.raw_transaction)
        notify(f"🚀 *DISPARO:* {w3.to_hex(h)}")
    except Exception as e: notify(f"🛡️ *Red:* {e}")

# --- INICIO ---
timer_10m = time.time()
notify("🚀 *AstraliX Hyper-Caza Online*", buttons=True)
            ...
            for n1, a1 in DEXs.items():
                for n2, a2 in DEXs.items():
                    ...
                    if neto > FILTRO_RADAR: 
                        ...
                        # VISTA REPLIT EN LOGS DE RAILWAY
                        print(f"📡 Radar: {name:<5} | Profit Neto: ${neto:.3f}")
