...
GAS_LIMIT = 500000                
ESPERA_ENTRE_BLOQUES = 3 # Con QuickNode podemos bajar a 3 segundos de forma segura
...
TG_TOKEN = os.environ.get('TELEGRAM_TOKEN', '8783847744:AAHdwwlEqP7HCgSXoFxRdD8snr5FRhT1OUo').strip()
TG_ID = os.environ.get('TELEGRAM_CHAT_ID', '6580309816').strip()
...
def execute_strike(target_token):
    ...
    try:
        ...
        notify(f"🛒 *COMPRA OK!* Hash: {w3.to_hex(h_buy)[:10]}...")
        ...
        if bal > 0:
            p_app_s = meme_c.encodeABI(fn_name="approve", args=[PANCAKE_ROUTER, bal])
            p_swp_s = router_c.encodeABI(fn_name="swapExactTokensForTokensSupportingFeeOnTransferTokens", args=[bal, 0, [target_token, WBNB_ADDR], CONTRATO_ADDR, int(time.time()) + 120])
            tx_s = apex_c.functions.apexStrike([target_token, PANCAKE_ROUTER], [w3.to_bytes(hexstr=p_app_s), w3.to_bytes(hexstr=p_swp_s)], [0, 0], 0).build_transaction({
                'from': MI_BILLETERA, 'nonce': w3.eth.get_transaction_count(MI_BILLETERA), 'gas': GAS_LIMIT, 'gasPrice': int(w3.eth.gas_price * 1.3)
            })
...
def scan(last_b):
    try:
        ...
        if now_b > last_b:
            # Escaneo blindado con QuickNode
            ...
            return now_b
    except Exception as e: print(f"⚠️ Error red: {e}")
    return last_b

# --- 🔥 INICIO ---
print("🚀 Motor TrenchBot V3.7 (QuickNode Edition) Iniciando...")
if w3.is_connected():
    print(f"✅ Conexión Privada Exitosa. Nodo: {RPC_URL.split('/')[2]}")
    notify("💰 *TRENCHBOT V3.7 ONLINE*\nConexión privada establecida.")
    ...
    print("❌ Error: No se pudo conectar al RPC de QuickNode.")