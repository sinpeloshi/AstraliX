...
if w3.is_connected():
    print("✅ Conectado a la Binance Smart Chain (BSC)")
else:
    print("❌ Error de conexión al RPC de Binance")
...
def ejecutar_retiro():
    ...
    try:
        ...
        notify(f"❌ *Fallo al retirar:* {str(e)[:100]}")
...
def execute_trade(r1, r2, t_addr, t_name, n1, n2, capital, mode="ARBI"):
    ...
    try:
        ...
        tx = contrato.functions.ejecutarArbitraje(r1, r2, WBNB_ADDR, t_addr, w3.to_wei(capital, 'ether')).build_transaction({
            ...
            'nonce': w3.eth.get_transaction_count(MI_BILLETERA),
            'gas': GAS_LIMIT,
            'gasPrice': int(w3.eth.gas_price * 1.05) 
        })
