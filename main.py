...
def execute_strike(target_token):
    ...
    try:
        ...
        tx = apex_c.functions.apexStrike([WBNB_ADDR, PANCAKE_ROUTER], [w3.to_bytes(hexstr=p_app), w3.to_bytes(hexstr=p_swp)], [0, 0], 0).build_transaction({
            ...
            'gas': GAS_LIMIT, 'gasPrice': int(w3.eth.gas_price * 1.3)
        })
...
def scan(last_b):
    try:
        ...
        if now_b > last_b:
            # Solo escaneamos un rango pequeño para evitar el error 413
        ...
        print(f"⚠️ Error en scan: {e}")
    return last_b

# --- 🔥 INICIO ---
print("🚀 Motor TrenchBot V3.8 Iniciando...")
if w3.is_connected():
    # FIX: Solo escaneamos los últimos 10 bloques al arrancar
    last_block = w3.eth.block_number - 10 
    print(f"✅ Conectado a QuickNode. Patrullando desde el bloque {last_block}...")
    notify("💰 *TRENCHBOT V3.8 ONLINE*")
    ...
    print("❌ Error de conexión al RPC.")