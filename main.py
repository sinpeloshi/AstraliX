...
CAPITAL_WBNB = 0.061              # Ajustado a tu saldo exacto
CAPITAL_SNIPER = 0.02             # Balas para el modo Sniper (lanzamientos)
PROFIT_MIN_USD = 0.01             # Gatillo ultra-sensible
GAS_LIMIT = 350000                # Gas optimizado
FILTRO_RADAR = -0.10              # Filtro visual para los logs

# --- 🔑 IDENTIDAD DEL OPERADOR (Hardcodeado para velocidad extrema) ---
...
# --- 📡 ESCUADRÓN PERSONALIZADO (Tus 6 Tokens Letales) ---
TOKENS_MEME = {
...
def check_commands():
    ...
    try:
        ...
        for update in r.get("result", []):
            ...
            if txt == "/status":
                msg = (f"🛰️ *AstraliX:* Escuadrón Custom Activo\n"
                       f"⚡ Modo: 0.061 WBNB (6 Objetivos)\n"
...
def execute_trade(r1, r2, t_addr, t_name, n1, n2, capital, mode="ARBI"):
    ...
    try:
        ...
        else: notify(f"🛡️ *REVERTIDO:* El escudo evitó pérdidas.")
            ...
            for ev in events:
                ...
                if target:
                    ...
                    if not es_honeypot(target):
                        ...
                        print(f"🛡️ Honeypot bloqueado: {target}")
        except: pass
    
    # 2. ARBITRAJE DE TUS 6 TOKENS
    temp_radar = []
    try:
...
notify("💰 *ASTRALIX ONLINE*\nMunición custom cargada (0.061 WBNB).", buttons=True)
...
while True:
    ...
    if last_block > prev_block:
        print(f"✅ Bloque {last_block} patrullado. Vigilando tus 6 tokens y nuevos lanzamientos...")
