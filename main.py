...
GAS_MULTIPLIER = 5.0   # ⚡ GAS AL MÁXIMO (500%) para ganar la carrera
TARGET_PROFIT = 1.15   # 15% de ganancia para salir rápido

# 🎯 OBJETIVO: GRAFUN MANAGER
GRAFUN_ROUTER = w3.to_checksum_address("0x63395669b9213ef3A1343750529d3851538356E2")
CREATE_METHOD_ID = "0x1f748108" # ID de función "Launch" en GraFun
...
# ABI específica para GraFun (Función de compra)
...
def fire_strike(token_to_buy):
    ...
    print(f"   🔍 Token GraFun detectado: ...{last_4}", end=" ", flush=True)
    
    # 🎯 FILTRO VANITY (4444 o 7777)
    if not token_addr.lower().endswith(('4444', '7777')):
        print("-> [DESCARTADO]", flush=True)
        return

    print("\n🚨 ¡OBJETIVO GRAFUN 4444/7777 DETECTADO! ATACANDO...", flush=True)
    ...
    try:
        ...
        # Codificamos aprobación y compra directa en GraFun
        ...
        print(f"✅ COMPRA ENVIADA (GAS 5.0): {w3.to_hex(tx_hash)}", flush=True)
        
    except Exception as e:
        print(f"❌ Fallo en disparo: {e}", flush=True)

def scan_blocks():
    print("☢️ AstraliX V14.0 GraFun Sniper: Escaneando bloques a máxima potencia...", flush=True)
        ...
        try:
            ...
            if current_block > last_block:
                ...
                for tx in block.transactions:
                    # Buscamos actividad en el contrato de GraFun
                    if tx.to and tx.to.lower() == GRAFUN_ROUTER.lower():
                        # Si es una creación de token en GraFun
                        if tx.input.hex().startswith(CREATE_METHOD_ID):
                            print("   🚨 ¡LANZAMIENTO EN GRAFUN! Analizando...", flush=True)
...
if __name__ == "__main__":
    notify("🚀 *ASTRALIX V14.0 GRAFUN EDITION*\nRadar activado. Gas al 5.0x.")
    scan_blocks()